...
install.packages("validate") install.packages("plotly") install.packages("webr") install.packages("gtsummary") install.packages("ggstatsplot")
...
names(danej) <- c("Plec", "W_zwiazku_malzenskim", "Osoby_na_utrzymaniu", "Edukacja", "Samozatrudnienie", "Dochód_aplikanta", "Dochod_wspolaplikanta", "Wysokosc_kredytu", "Okres_kredytu", "Historia_kredytowa", "Obszar_zamieszkania")
...
Wykres pokazuje, że osoby w związku małżeńskim częściej otrzymują kredyt niż osoby niezamężne. Liczba odmów jest stosunkowo niska w obu grupach.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(danej, aes(x = Dochód_aplikanta, 
...
Czerwona linia regresji "Nie przyznano" wykazuje się tendencją malejącą — wyższe dochody aplikanta wiążą się z niższymi dochodami współaplikanta, gdy kredyt nie został przyznany.
...
Kredyty, które zostały przyznane są rozproszone w zakresie zarówno kwot kredytów, jak i okresu spłaty, natomiast większość przyznanych kredytów jest objęta dłuższym okresem spłaty.

Kredyty, które zostały odrzucone są bardziej skupione w określonych przedziałach kwoty i okresu.

4.  Wykres pudełkowy przedstawia zależność między dochodami aplikantów a ich historią kredytową (przyznanie lub nieprzyznanie kredytu).

```{r echo=FALSE}
library(ggstatsplot)
ggbetweenstats(data = danej, x = Historia_kredytowa, y = Dochód_aplikanta)
max(danej$Dochód_aplikanta)
dane_bez_maksimum <- danej[-82,]
ggbetweenstats(data = dane_bez_maksimum, x = Historia_kredytowa, y = Dochód_aplikanta)
library(ggplot2)
