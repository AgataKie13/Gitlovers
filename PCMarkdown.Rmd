...
dane2$Gender <- ifelse(is.na(dane2$Gender), NA, 
                       ifelse(dane2$Gender == "Male", 1, 
                            ifelse(dane2$Gender == "Female", 0, dane2$Gender)))

dane2$Married <- ifelse(is.na(dane2$Married), NA, 
                        ifelse(dane2$Married == "Yes", 1, 
                              ifelse(dane2$Married == "No", 0, dane2$Married)))

dane2$Self_Employed <- ifelse(is.na(dane2$Self_Employed), NA, 
                              ifelse(dane2$Self_Employed == "Yes", 1, 
                                  ifelse(dane2$Self_Employed == "No", 0, dane2$Self_Employed)))


dane2$Education <- ifelse(is.na(dane2$Education), NA, 
                          ifelse(dane2$Education == "Graduate", 1, 
                              ifelse(dane2$Education == "Not Graduate", 0, dane2$Education)))


dane2$Property_Area <- ifelse(is.na(dane2$Property_Area), NA, 
                            ifelse(dane2$Property_Area == "Rural", 1, 
                                ifelse(dane2$Property_Area == "Semiurban", 2, 
                                  ifelse(dane2$Property_Area == "Urban", 3, NA))))


dane2$Dependents <- ifelse(is.na(dane2$Dependents), NA, 
                           ifelse(dane2$Dependents == "3+", 3, 
                                  as.numeric(dane2$Dependents)))
```
