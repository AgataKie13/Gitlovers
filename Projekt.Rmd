...
W analizowanym zbiorze danych znajduje się `r sum(is.na(dane))` wartości oznaczonych jako NA oraz `r sum(dane == "", na.rm = TRUE)` zmienne puste. Oznacza to, że w sumie są `r sum(dane == "" | is.na(dane), na.rm = TRUE)` zmienne nieznane. W związku z tym należy przekodować zmienne tak, aby w miejsce pustych komórek pojawiły się wartości NA.
...
Ostatecznie w danych znajdują się `r sum(is.na(dane2))` braki danych oznaczone jako NA.
...
gg_miss_fct(dane2, Credit_History) + 
  ggtitle("Wykres słupkowy braków danych względem kategorii")
```
<center> **Diagram UpSet dla brakujących danych** </center>
```{r echo=FALSE, message=FALSE, warning=FALSE}
gg_miss_upset(dane2)
...
library(kableExtra)
wynik_shapiro <- shapiro.test(danej$Historia_kredytowa)
wyniki_tabela <- data.frame(
  Statystyka = wynik_shapiro$statistic,
  Wartość_p = wynik_shapiro$p.value
)
wyniki_tabela %>%
  kable("html", caption = "Wyniki testu Shapiro-Wilka") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
```{r echo=FALSE}
logit <- glm(Historia_kredytowa ~ Dochod_aplikanta + Wysokosc_kredytu + Dochod_wspolaplikanta + Okres_kredytu + Plec + W_zwiazku_malzenskim  + Osoby_na_utrzymaniu  + Edukacja + Samozatrudnienie + Obszar_zamieszkania, data = danej, family = binomial)
summary(logit)$coefficients
```
Z analizy istotności statystycznej współczynników wynika, że tylko zmienna **Dochód aplikanta** jest istotna.
W związku z tym zbudowano drugi model z uwzględnieniem tylko tej zmiennej.
```{r echo=FALSE}
logit1 <- glm(Historia_kredytowa ~ Dochod_aplikanta, data = danej, family = binomial)
summary(logit1)$coefficients
```

**Pierwszy model** logitowy pokazuje, że tylko jedna cecha ma istotny wpływ na zmienną zależną. W związku z tym zbudowano **drugi model**, który przedstawia tylko zależność między **historią kredytową** a **dochodem aplikanta**. Również analiza wartości **AIC** pokazuje, że drugi model jest lepiej dopasowany:\
AIC dla modelu 1 = `r summary(logit)$aic`\
AIC dla modelu 2 = `r summary(logit1)$aic`
