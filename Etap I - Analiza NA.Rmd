...
PieDonut(data = onuk, aes(Osoby_na_utrzymaniu, Historia_kredytowa, count = n))
...
ggplot(danej, aes(x = Married, fill = factor(Credit_History))) +
...
```{r}
ggplot(danej, aes(x = ApplicantIncome, 
                 y = CoapplicantIncome, 
                 color = factor(Credit_History))) +
  geom_point() +
...
```{r}
ggplot(danej, aes(x = factor(Credit_History), 
                 y = ApplicantIncome, 
