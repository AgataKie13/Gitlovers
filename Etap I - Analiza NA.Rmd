...
## b) Imputacja brakujących danych metodą hot-deck
```{r}
library(hot.deck)
...
## c) Imputacja brakujących danych metodą KNN - K-najbliższych sąsiadów

```{r}
install.packages("VIM")
library(VIM)

dane_KNN <- kNN(dane2, k = 5, imp_var = FALSE)
```
...
sum(is.na(dane_KNN))
```
## d). Wielowymiarowe wypełnianie braków danych przez równania łańcuchowe
...
Gender_imputed <- mice(dane2[, c("Gender", "Married", "Dependents", "Education", "Self_Employed", 
...
```{r}
Dependents_imputed <- mice(dane2[, c("Dependents", "Gender", "Married", "Education", "Self_Employed", 
                                    "ApplicantIncome", "CoapplicantIncome", "LoanAmount", 
                                    "Loan_Amount_Term", "Credit_History", "Property_Area")], 
                           m = 5, method = "pmm")
...
```{r}
Self_Employed_imputed <- mice(dane2[, c("Self_Employed", "Gender", "Married", "Dependents", 
...
```{r}
LoanAmount_imputed <- mice(dane2[, c("LoanAmount", "Gender", "Married", "Dependents", 
...
```{r}
Loan_Amount_Term_imputed <- mice(dane2[, c("Loan_Amount_Term", "Gender", "Married", "Dependents", 
                                          "Education", "Self_Employed", "ApplicantIncome", 
...
```{r}
Credit_History_imputed <- mice(dane2[, c("Credit_History", "Gender", "Married", "Dependents", 
                                        "Education", "Self_Employed", "ApplicantIncome", 
