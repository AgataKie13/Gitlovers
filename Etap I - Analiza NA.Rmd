...
install.packages("validate") install.packages("plotly") install.packages("webr") install.packages("gtsummary") install.packages("ggstatsplot") install.packages("ggplot2")
...
danem %>%
  ...
  modify_caption("**Tabela 1. Statystyki opisowe według historii kredytowej po imputacji**") %>%
  modify_footnote(
...
cooks.distance(fit3)
plot(fit3,which=4, main="Kwota kredytu vs Dochód wnioskodawcy")

fit1 = lm(LoanAmount ~ Loan_Amount_Term,data=dane2)
cooks.distance(fit4)
plot(fit4,which=4, main="Kwota kredytu vs Czas trwania")

fit2 = lm(LoanAmount ~ Married,data=dane2)
cooks.distance(fit)
plot(fit,which=4, main="Kwota kredytu vs Stan cywilny")

fit3 = lm(LoanAmount ~ Gender,data=dane2)
cooks.distance(fit1)
plot(fit1,which=4, main="Kwota kredytu vs Płeć")

layout(matrix(c(1), nrow = TRUE, byrow = 1), heights = c(1, 1, 1))

fit4 = lm(LoanAmount ~ Education,data=dane2)
cooks.distance(fit2)
plot(fit2,which=4, main="Kwota kredytu vs Wykształcenie")
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# 1. Wykres kolumnowy ukazujący historię kredytową w zależności od stanu cywilnego.
library(tidyverse)
ggplot(danej, aes(x = W_zwiazku_malzenskim, fill = factor(Historia_kredytowa))) +
  ...
  scale_fill_manual(name = "Historia kredytowa",
                    values = c("Nie" = "red", "Tak" = "green"),
                    labels = c("Nie przyznano", "Przyznano")) +
  ggtitle("Historia kredytowa w zależności od stanu cywilnego") +
  theme_light()

# 2. Wykres punktowy przedstawiający zależność między dochodami aplikanta i współaplikanta, z podziałem na historię kredytową („Przyznano” i „Nie przyznano”).
...
# 3. Wykres punktowy ukazujący zależność między historią kredytową a kwotą kredytu oraz okresem kredytowania.
...
Rozpoczęto od stworzenia wykresu pudełkowy przedstawiającego zależność między dochodami aplikantów a ich historią kredytową (przyznanie lub nieprzyznanie kredytu). Następnie przeprowadzono wstępne wnioskowanie statystyczne.
...
ggbetweenstats(data = dane_bez_maksimum, x = Historia_kredytowa, y = Dochod_aplikanta)
...
hist(danej$Historia_kredytowa)

shapiro.test(danej$Historia_kredytowa)
...
wilcox.test(danej$Historia_kredytowa, danej$Dochod_aplikanta)


# 3. Test ANOVA - sprawdza, czy istnieją istotne różnice między więcej niż dwoma grupami.
wyniki <- aov(danej$Historia_kredytowa ~ danej$Dochod_aplikanta)
summary(wyniki)


model <- lm(danej$Historia_kredytowa ~ danej$Dochod_aplikanta)
summary(model)

ggscatterstats(
  data=danej,
  y=Dochod_aplikanta,
  x=Historia_kredytowa,
  type="np"
)
...
- trzy/4/5 testów
alfa=0.01 (błąd pierwszego rodzaju - odrzucenie hipotezy która jest tak na prawdę prawdziwa)
H0: u1=u2 ; u1-u2=0

tstudent t=(średnia1-średnia2)/pierwiastek(odchylenie standoardowe/n1 + odst/n2)

z
chi2
fisher

złota reguła decyzyjna: pole powierzchni < alfa 

beta (błąd drugiego rodzaju - prawd. przyjęcia fałszu)
beta <=0.1
1-beta = moc testu (power..test) =  chcemy żeby była 0.9 (90%)
power analisys - po testowaniu


testy na danych jakościowych
  kobiety i mężczyzn i ankieta 0-5
  
  tabela kontyngencji - analiza danych jakościowych - nie policzy się średniej - czy proporcjonalnie idzie podobnie od 1 do 5 czy inaczej
  1. tabela źródłowa
  2. druga tabela - tzw. test niezależności - tabelka oczekiwana - "wypadkowa oczekiwana" suma wiersza*suma wiersza/n
  3. suma kwadratów chi2 = suma (((obs-oczek)^2)/oczekiwana)
    gdy się zwiększa to znaczy, że jest inaczej niż neutralnie. im większe chi2 tym więcej dowodów że nie jest przypadkowo, niezależnie, że kobiety inaczej odpowiadały niż mężczyźni
      gdy chi2 wpadnie w ogonek 
      0->niesk, asymetria lewostr.
...
hist(diamonds$price)  #nie można używać średnich przy takim rozkładzie jak już to mediana, rangowanie

ggstatsplot::ggbetweenstats(
  data=diamonds,
  y=price, #ilościowa
  x=cut, #jakościowa
  type="np"  #p-paramtreyczny - średnia (domyślne nie trzeba pisać jeśli rozkład jest symetryczny/normalny) ; np-nieparamteryczny - bez użycia parametrów takich jak średnia
)
    #czerwona korpeczka to mediana; !klamerki - pary które istotnie różnią się między sobą

wyniki <- aov(data=diamonds,price~cut)
summary(wyniki)

wyniki2 <- aov(data=diamonds,price~cut*color) #szukanie nie tylko dla oddzielnie tych zmiennych ale też interakcji między zmiennymi
summary(wyniki2)
...
Końcowym etapem była wizualizacja danych i wnioskowanie statystyczne, które pozwoliły na identyfikację kluczowych wzorców i zależności między zmiennymi...
